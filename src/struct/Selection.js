...
module.exports = class Selection {
  ...
  async execute (message, rows) {
    ...
    try {
      ...
      if (message.guild && message.channel.permissionsFor(message.guild.me).has('MANAGE_MESSAGES')) { response.delete() }
    } catch (err) {
      if (err instanceof Error) { error(err) } else { message.util.edit('Selection expired.', { embed: null }) }
    }
    this.users.delete(message.author.id)
